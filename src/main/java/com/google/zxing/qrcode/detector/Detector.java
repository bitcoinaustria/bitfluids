...
public class Detector {
  ...
  public static PerspectiveTransform createTransform(ResultPoint topLeft, ResultPoint topRight, ResultPoint bottomLeft,
    ...
    return PerspectiveTransform.quadrilateralToQuadrilateral(3.5f, 3.5f, dimMinusThree, 3.5f, sourceBottomRightX,
        sourceBottomRightY, 3.5f, dimMinusThree, topLeft.getX(), topLeft.getY(), topRight.getX(), topRight.getY(),
   ...
   * </p>
   */
  protected static int computeDimension(ResultPoint topLeft, ResultPoint topRight, ResultPoint bottomLeft,
   ...
   * total width of a finder pattern by looking for a black-white-black run from
   * the center in the direction of another point (another finder pattern
   ...
   * it finds white, then black, then white again. It reports the distance from
   * the start to this point.
   * </p>
   * 
   * <p>
  ...
  private float sizeOfBlackWhiteBlackRun(int fromX, int fromY, int toX, int toY) {
    ...
    for (int x = fromX, y = fromY; x != toX; x += xstep) {
      ...
      // FIXME(dswitkin): This method seems to assume square images, which
      // can cause these calls to
      ...
      // Found black, white, black, and stumbled back onto white, so we're
      // done.
      if (state == 3) {
        int diffX = x - fromX;
        int diffY = y - fromY;
   ...
   */
  protected AlignmentPattern findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX, int estAlignmentY,
    ...
    AlignmentPatternFinder alignmentFinder = new AlignmentPatternFinder(image, alignmentAreaLeftX, alignmentAreaTopY,
