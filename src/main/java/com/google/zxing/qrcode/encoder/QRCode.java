...
public final class QRCode {
  ...
  public boolean isValid() {
    ...
    mode != null && ecLevel != null && version != -1 && matrixWidth != -1 && maskPattern != -1 && numTotalBytes != -1
        && numDataBytes != -1 && numECBytes != -1 && numRSBlocks != -1 &&
   ...
   * (int y = 0; y < matrix.height(); ++y) { for (int x = 0; x < matrix.width();
   * ++x) { int value = matrix.get(y, x); if (!(value == 0 || value == 1)) { //
   * Found non zero/one value. return false; } } } return true; }
   */
