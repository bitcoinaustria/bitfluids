...
public final class Encoder {
   ...
   * as a hint; if it is Shift_JIS, and the input is only double-byte Kanji,
   * then we return {@link Mode#KANJI}.
   */
  ...
  private static int chooseMaskPattern(BitArray bits, ErrorCorrectionLevel ecLevel, int version, ByteMatrix matrix)
      throws WriterException {
  ...
  /**
   * Initialize "qrCode" according to "numInputBytes", "ecLevel", and "mode". On
   * success, modify "qrCode".
   */
   ...
   */
  static void interleaveWithECBytes(BitArray bits, int numTotalBytes, int numDataBytes, int numRSBlocks, BitArray result)
      throws WriterException {
