...
public abstract class Binarizer {
   ...
   * decoding 1D barcodes and may choose to apply sharpening. For callers which
   * only examine one row of pixels at a time, the same BitArray should be
   * reused and passed in with each call for performance. However it is legal to
   * keep more than one row at a time if needed.
   * 
   * @param y
   *          The row to fetch, 0 <= y < bitmap height.
   * @param row
   *          An optional preallocated array. If null or too small, it will be
   *          ignored. If used, the Binarizer will call BitArray.clear(). Always
   ...
   * may be stateful, e.g. keeping a cache of 1 bit data. See Effective Java for
   * why we can't use Java's clone() method.
   * 
   * @param source
